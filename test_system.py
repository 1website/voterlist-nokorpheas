...
def run_tests():
    ...
    # Ensure Geography is hidden in sidebar for viewer but Dashboard is visible
    assert "រចនាសម្ព័ន្ធភូមិសាស្ត្រ" not in res_v_rep.text
    assert "ផ្ទាំងគ្រប់គ្រងស្ថិតិ" in res_v_rep.text
    assert "របាយការណ៍ &amp; សុវត្ថិភាព" in res_v_rep.text or "របាយការណ៍ & សុវត្ថិភាព" in res_v_rep.text
    print("[PASS] 28. Viewer logged in and accessed Dashboard and Reports successfully")
